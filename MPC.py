...
class FixedDifferentialDriveMPC:
    ...
    def get_reference_trajectory(self, current_step):
        ...
        for k in range(self.N + 1):
            ...
            # 参考位置 - 正弦波
            x_ref = speed * t
            y_ref = amplitude * np.sin(frequency * t)
            
          
            if k == 0:
   
                dx_dt = speed
                dy_dt = amplitude * frequency * np.cos(frequency * t)
                theta_ref = np.arctan2(dy_dt, dx_dt)
                if(t==0):
                    self.theta_init=theta_ref
                    self.state = np.array([0, 0.0, theta_ref])
                   
            else:
                # 使用有限差分计算切线方向
                t_prev = (current_step + k - 1) * self.T
                x_prev = speed * t_prev
                y_prev = amplitude * np.sin(frequency * t_prev)
            ...
            if k < self.N:
                ...
                x_next = speed * t_next
                y_next = amplitude * np.sin(frequency * t_next)
                
                # 计算曲率
                dx1 = x_ref - x_prev if k > 0 else speed
